    ...
    match system::perform_recovery().await {
        Ok(report) => {
            ...
            if passed == total {
                ...
                println!("{}", format!("⚠️  {}/{} checks passed", passed, total).yellow().bold());
            }
            ...
            match health {
                ...
                system::HealthStatus::Degraded(msg) => {
                    println!("{}", format!("⚠️  System is degraded: {}", msg).yellow().bold());
                }
                system::HealthStatus::Unhealthy(msg) => {
                    println!("{}", format!("❌ System is unhealthy: {}", msg).red().bold());
                }
            }
        }
        Err(e) => {
