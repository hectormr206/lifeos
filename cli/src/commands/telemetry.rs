...
async fn cmd_stats() -> anyhow::Result<()> {
    let body: serde_json::Value = daemon_client::get_json("/api/v1/telemetry/stats")
        .await
        .inspect_err(|e| {
...
async fn cmd_consent(level: Option<String>) -> anyhow::Result<()> {
    if let Some(level) = level {
        ...
        let _: serde_json::Value = daemon_client::post_json("/api/v1/telemetry/consent", &payload)
            .await
            .inspect_err(|e| {
                if e.to_string().contains("is lifeosd running") {
                    print_daemon_down();
        ...
        let body: serde_json::Value = daemon_client::get_json("/api/v1/telemetry/consent")
            .await
            .inspect_err(|e| {
                if e.to_string().contains("is lifeosd running") {
                    print_daemon_down();
...
async fn cmd_events(limit: usize) -> anyhow::Result<()> {
    let body: serde_json::Value = daemon_client::get_json("/api/v1/telemetry/events")
        .await
        .inspect_err(|e| {
            if e.to_string().contains("is lifeosd running") {
                print_daemon_down();
...
async fn cmd_snapshot() -> anyhow::Result<()> {
    let body: serde_json::Value = daemon_client::post_empty("/api/v1/telemetry/snapshot")
        .await
        .inspect_err(|e| {
            if e.to_string().contains("is lifeosd running") {
                print_daemon_down();
...
async fn cmd_export() -> anyhow::Result<()> {
    let body: serde_json::Value = daemon_client::get_json("/api/v1/telemetry/export")
        .await
        .inspect_err(|e| {
            if e.to_string().contains("is lifeosd running") {
                print_daemon_down();
...
async fn cmd_clear() -> anyhow::Result<()> {
    let _: serde_json::Value = daemon_client::post_empty("/api/v1/telemetry/clear")
        .await
        .inspect_err(|e| {
            if e.to_string().contains("is lifeosd running") {
                print_daemon_down();
