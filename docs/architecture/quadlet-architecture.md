...
> **Status update (Phase 8b):** `lifeos-lifeosd` migrated from `Network=host` to `lifeos-net.network`; sibling containers (`lifeos-llama-server`, `lifeos-llama-embeddings`, `lifeos-tts`, `lifeos-simplex-bridge`) are also on `lifeos-net.network` (their migrations landed earlier). The full Quadlet stack is bridged.
>
> The lifeosd container exports `LIFEOS_API_BIND=0.0.0.0:8081`. Inside the bridge netns the container's own `127.0.0.1` is unreachable from the host, so the daemon must listen on the wildcard for `PublishPort=127.0.0.1:8081:8081` to forward traffic into it. The bootstrap-token endpoint stays loopback-only by combining: (1) `PublishPort=127.0.0.1:...` so external interfaces never reach the listener; (2) a fail-closed peer-address loopback check (axum is served via `into_make_service_with_connect_info::<SocketAddr>()`, and the handler rejects requests with no `ConnectInfo` instead of falling through); (3) a required `Origin` header that must address loopback when present; (4) a `Host` header check for loopback to defeat DNS rebinding. The previous "triple-guard" claim was a double-guard until round-2 JD wired `ConnectInfo` correctly — the documentation now reflects what the code actually enforces.
>
> Sibling URLs are resolved at startup by `daemon/src/endpoints.rs::{llama_url, embeddings_url, tts_url}`, which read `LIFEOS_LLAMA_URL`, `LIFEOS_EMBED_URL`, and `LIFEOS_TTS_URL` (with `LIFEOS_TTS_SERVER_URL` as a legacy alias that emits a deprecation warning when used). Each URL is resolved exactly once per process via `OnceLock` and logged at INFO so the journal records what the daemon actually talks to. Defaults remain `http://127.0.0.1:80xx` for `Network=host` rollback. Values must be bare `scheme://host[:port]` — paths/queries/empty authorities/embedded whitespace are rejected with a `WARN` log. **Changing these values requires a `systemctl restart lifeos-lifeosd`.**
