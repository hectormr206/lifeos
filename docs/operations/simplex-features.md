...
mode on `ws://127.0.0.1:5226`. As of 0.8.36 the bot runs as a podman
...
rollback target (no longer auto-starting). All messages are dispatched
...
6. Kokoro (`lifeos-tts.service` on `127.0.0.1:8084`) synthesizes the
