...
/// Returns `true` when `systemctl --user cat <unit>` output indicates the unit
/// was generated by Quadlet (i.e. it lives under
/// `/run/user/<UID>/systemd/generator/`).  The first line of `systemctl cat`
/// output is a comment with the source path; Quadlet-generated units always
/// appear there.
pub fn is_quadlet_generated(systemctl_cat_output: &str) -> bool {
    systemctl_cat_output
        .lines()
        .next()
        .map(|first_line| {
            let trimmed = first_line.trim();
            trimmed.starts_with("# /run/user/") && trimmed.contains("/systemd/generator/")
        })
        .unwrap_or(false)
}

/// Checks whether the running `lifeos-llama-server.service` was generated by
/// Quadlet by running `systemctl --user cat` and inspecting its output.
fn quadlet_unit_is_active() -> bool {
    Command::new("systemctl")
        .args(["--user", "cat", USER_LLAMA_UNIT_NAME])
        .output()
        .map(|out| {
            let stdout = String::from_utf8_lossy(&out.stdout);
            is_quadlet_generated(&stdout)
        })
        .unwrap_or(false)
}

/// Testable inner implementation of the user-fallback service logic.
///
/// - `unit_path`: path where the `.service` file should be written.
/// - `content`: the service file content to write.
/// - `quadlet_checker`: closure that returns `true` if a Quadlet-generated
///   unit is already active.  In production this calls `quadlet_unit_is_active`;
///   in tests it can be any closure.
/// - `daemon_reload_called`: output flag set to `true` when
///   `systemctl --user daemon-reload` would be invoked.
///   In production, when this flag is set, the caller runs the real reload.
///
/// Returns `Ok(true)` when the file was written (content changed or new),
/// `Ok(false)` when skipped (quadlet detected or content unchanged).
fn ensure_user_llama_service_inner(
    unit_path: &Path,
    content: &str,
    quadlet_checker: impl Fn() -> bool,
    daemon_reload_called: &mut bool,
) -> Result<()> {
    if quadlet_checker() {
        info!(
            "[ai_runtime] Quadlet-generated {} detected; skipping legacy fallback. \
             Configure env via the .container file's [Container] Environment= or EnvironmentFile=.",
            USER_LLAMA_UNIT_NAME
        );
        return Ok(());
    }
