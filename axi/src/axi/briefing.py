...
_FINAL_SYNTHESIS_PROMPT = (
    "Ya tenés suficiente información de las búsquedas anteriores. NO busques "
    "más. Devolvé AHORA únicamente el objeto JSON del boletín, con los ítems "
    "más recientes y relevantes que encontraste (cada uno con su URL real). Si "
    "no encontraste nada actual, devolvé pocos ítems o una lista vacía y "
    "explicalo en el resumen. No inventes noticias ni URLs."
)
...
def build_briefing_system(today: str) -> str:
    ...
    """
    return (
        "Sos un asistente que prepara boletines curados. "
        ...
        "(c) si querés acotar por tiempo usá time_range='week' o 'month'. "
        "IMPORTANTE: NO uses time_range='day' — en este índice suele devolver "
        "CERO resultados. Si una búsqueda vuelve vacía, reintentá con una query "
        "más simple o sin time_range.\n"
        "Regla de frescura: priorizá SIEMPRE lo más reciente. El buscador no "
        "siempre trae la fecha exacta de cada ítem; usá las fechas y pistas que "
        ...
        "pedidos, devolvé MENOS en vez de rellenar con viejos. NUNCA inventes "
