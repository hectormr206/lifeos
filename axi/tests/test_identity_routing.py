...
class TestIdentityRoutingEndToEnd:
    ...
    def test_register_alias_merge_forwarded(self):
        ...
        canonical = identity.ensure_entity("Celia García Mateo", "person")
        dup = identity.ensure_entity("Cely", "person")
        assert canonical != dup
        # Give the duplicate an edge so we can prove it is re-pointed, not lost.
        fact = store.add_node(kind="fact", label="quiere a Cely")
        store.add_edge(fact, dup, "mentions")

        identity.register_alias("Celia García Mateo", "Cely", "person")
        ...
        assert "Cely" in data.get("aliases", [])

    def test_add_relation_forwarded(self):
        identity.add_relation("esposa", "Celia García Mateo", "person")
        c = store._connect()
        hub = c.execute(
        ...
        assert ent["label"] == "Celia García Mateo"
    ...
    def test_link_fact_to_entities_forwarded(self):
        ent = identity.ensure_entity("Celia García Mateo", "person")
        fact = store.add_node(kind="fact", label="cené con Celia García Mateo")
        identity.link_fact_to_entities(fact, "cené con Celia García Mateo")
...
class TestIdentityConnProvidedNotForwarded:
    ...
    def test_register_alias_with_conn_not_forwarded(self, _spy_forwards):
        conn = store._connect()
        canonical = identity.ensure_entity("Celia García Mateo", "person", conn=conn)
        identity.register_alias("Celia García Mateo", "Cely", "person", conn=conn)
        assert not any(op.startswith("identity.") for op in _spy_forwards)
        import json
        data = json.loads(
            conn.execute(
