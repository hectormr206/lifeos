...
def test_extract_and_store_hypertension_entity_graph(monkeypatch):
    ...
    raw = json.dumps({"facts": [], "relations": [
        ...
        {"subject": "hipertensión", "subject_kind": "condition",
         "relation": "diagnosticada_por", "object": "Dra. Tere", "object_kind": "person"},
        {"subject": "hipertensión", "subject_kind": "condition",
    ...
    doc = c.execute(
        "SELECT id FROM nodes WHERE kind='person' AND label='Dra. Tere'"
    ).fetchone()
...
def test_extract_old_shape_creates_user_to_entity_edge(monkeypatch):
    ...
    monkeypatch.setattr(config, "get", cfg)
    raw = json.dumps({"facts": [], "relations": [
        {"relation": "esposa", "entity": "Celia", "kind": "person"},
    ]}, ensure_ascii=False)
    monkeypatch.setattr(extractor, "brain_ask", lambda **kw: raw)
    extractor.extract_and_store("mi esposa Celia", "ok", None)
    c = store._connect()
    hub = identity.ensure_user_hub()
    celia = c.execute("SELECT id FROM nodes WHERE kind='person' AND label='Celia'").fetchone()
    assert celia is not None
    assert c.execute(
        "SELECT 1 FROM edges WHERE from_id=? AND to_id=? AND kind='esposa'",
        (hub, celia["id"]),
    ).fetchone() is not None
