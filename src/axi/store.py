...
CREATE TABLE IF NOT EXISTS bp_readings (
  ...
  pulse       INTEGER,
  context     TEXT,                  -- 'morning'|'after_meal'|'before_meds'|…
  notes       TEXT,
  node_id     INTEGER REFERENCES nodes(id) ON DELETE SET NULL,
  created_at  REAL NOT NULL
);
CREATE INDEX IF NOT EXISTS idx_bp_date ON bp_readings(date);

CREATE TABLE IF NOT EXISTS medications (
  id              INTEGER PRIMARY KEY AUTOINCREMENT,
  name            TEXT NOT NULL,
  dosage          TEXT NOT NULL,     -- '50mg'
  frequency       TEXT NOT NULL,     -- '1x daily', 'every 8h', …
  schedule_times  TEXT,              -- JSON array of HH:MM strings
  start_date      TEXT NOT NULL,
  end_date        TEXT,
  prescriber      TEXT,
  reason          TEXT,
  source_doc_id   INTEGER REFERENCES medical_documents(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS medical_documents (
  id                          INTEGER PRIMARY KEY AUTOINCREMENT,
  date                        TEXT NOT NULL,
  doc_type                    TEXT NOT NULL,  -- 'prescription'|'lab_result'|'xray'|'note'|…
...
CREATE INDEX IF NOT EXISTS idx_meddocs_date ON medical_documents(date);
